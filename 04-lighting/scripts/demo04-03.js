...
function loadTextures() {
    ...
    textures.push( gl.createTexture() );
    loadImageForTexture("textures/ft_stone01_n.png", textures[1]);
    textures.push( gl.createTexture() );
    loadImageForTexture("textures/ft_stone01_s_wet.png", textures[2]);
}
...
function initShaders() {
    ...
    gl_program_loc.uSpecMap = gl.getUniformLocation(gl_program, "uSpecMap");
    gl_program_loc.uDrawState = gl.getUniformLocation(gl_program, "uDrawState");
    gl_program_loc.uDrawSpecular = gl.getUniformLocation(gl_program, "uDrawSpecular");
}
...
function draw() {
    ...
    // Set texture2 to have the specmap
    gl.activeTexture(gl.TEXTURE2);;
    gl.bindTexture(gl.TEXTURE_2D, textures[2]);
    
    // Set uSpecMap to have the value 2, so it uses TEXTURE2
    gl.uniform1i(gl_program_loc.uSpecMap, 2);
    
    // Send the drawstate to the shader
    gl.uniform1i(gl_program_loc.uDrawState, drawState);
    gl.uniform1i(gl_program_loc.uDrawSpecular, drawSpecular);
...
function main() {
    ...
    // Add a simple key listeners to toggle between skin drawing, 
    // normal map drawing, and light intensity drawing
    window.onkeydown = function( event ) {
        // If 'space' is pressed, cycle through drawing states:
        //  0 -- Draw Color map with lighting using the normal map
        //  1 -- Draw Color map without lighting
        //  2 -- Draw Normal map (no lighting)
        //  3 -- Draw Specular map (no lighting)
        //  4 -- Draw light intensity 
        if( event.keyCode == 32 ) {
            drawState += 1;
            drawState = drawState % 5;
        }
        
        // If 's' is being held down, don't show the specular map:
        if( event.keyCode == 83 ) {
            drawSpecular = 0;
        }
    }
    
    // Add a keyup listener, so that we can tell when keys are released
    window.onkeyup = function( event ) {
        // If 's' is lifted, show the specular map again:
        if( event.keyCode == 83 ) {
            drawSpecular = 1;
        }
    }
