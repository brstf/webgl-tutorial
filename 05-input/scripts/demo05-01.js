...
function draw() {
    ...
    // Increment y and x rotation
    yrot += yinc;
    if ( Math.abs(yrot) >= 2 * Math.PI  ) {
        yrot = yrot % (2 * Math.PI);
    }
    xrot += xinc;
    if ( Math.abs(xrot) >= 2 * Math.PI  ) {
        xrot = xrot % (2 * Math.PI);
    }
...
function main() {
    ...
    window.onkeydown = function( event ) {
        // If 'right' is pressed, increase y-increment (rotate around y)
        if( event.keyCode == 39 ) {
            yinc = 0.1;
        }
        
        // If 'left' is pressed, decrease y-increment
        if( event.keyCode == 37 ) {
            yinc = -0.1;
        }
        
        // If 'up' is pressed, decrease x-increment
        if( event.keyCode == 38 ) {
            xinc = -0.1;
        }
        
        // If 'down' is pressed, increase x-increment
        if( event.keyCode == 40 ) {
            xinc = 0.1;
        }
    }
    ...
    window.onkeyup = function( event ) {
        // If 'right' is lifted, 0 y-increment
        if( event.keyCode == 39 ) {
            yinc = 0.0;
        }
        
        // If 'left' is lifted, 0 y-increment
        if( event.keyCode == 37 ) {
            yinc = 0.0;
        }
        
        // If 'up' is lifted, 0 x-increment
        if( event.keyCode == 38 ) {
            xinc = 0.0;
        }
        
        // If 'down' is lifted, 0 x-increment
        if( event.keyCode == 40 ) {
            xinc = 0.0;
        }
    }
